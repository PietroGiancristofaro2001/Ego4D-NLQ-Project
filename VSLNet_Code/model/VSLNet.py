...
class VSLNet(nn.Module):
    def __init__(self, configs, word_vectors):
        ...
        # query-guided highlighting- id doesn't work inizialize always module and after compute the flag
        # Control model type by using configs
        self.use_highlight_layer = (configs.model_type.lower() == 'vslnet')
        self.highlight_layer = None
        #check if we are using VSLnet or VSLbase
        if self.use_highlight_layer:
    ...
    def forward(self, word_ids, char_ids, video_features, v_mask, q_mask):
        ...
        features_f = self.cq_concat(features, query_features, q_mask)
        # Initialize h_score (will be None in VSLBase)
        h_score = None 
        # Default input (VSLBase: output di concat)
        features_for_predictor = features 
        #if we are using VSLnet it will be true
        if self.use_highlight_layer:
          h_score = self.highlight_layer(features, v_mask)
          features_for_predictor = features * h_score.unsqueeze(2)
        start_logits, end_logits = self.predictor(features_for_predictor, mask=v_mask)
